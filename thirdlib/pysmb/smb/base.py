...
class SMB(NMBSession):
    ...
    def __init__(self, username, password, my_name, remote_name, domain='', use_ntlm_v2=True,
        ...
        self.log.info(
            'Authentication with remote machine "%s" for user "%s" will be using NTLM %s authentication (%s extended security)',
            ...
            if i > 0:
                if not self.is_using_smb2:
                    self.log.debug('Received SMB message "%s" (command:0x%2X flags:0x%02X flags2:0x%04X TID:%d UID:%d)',
                                   ...
                                   self.smb_message.command, self.smb_message.flags, self.smb_message.flags2,
                                   self.smb_message.tid, self.smb_message.uid)
                else:
    ...
    def _sendSMBMessage_SMB2(self, smb_message):
        ...
        if self.is_signing_active:
            ...
            self.log.debug('MID is %d. Signature is %s. Total raw message is %d bytes', smb_message.mid,
                           binascii.hexlify(smb_message.signature), len(smb_message.raw_data))
        else:
            smb_message.raw_data = smb_message.encode()
                ...
                if message.status == 0:
                    ...
                    try:
                        ...
                        if result == securityblob.RESULT_ACCEPT_COMPLETED:
                            ...
                            raise ProtocolError(
                                'SMB2_COM_SESSION_SETUP status is 0 but security blob negResult value is %d' % result,
                                message.raw_data, message)
                    ...
                    self.has_authenticated = False
                    self.log.info(
                        'Authentication (on SMB2) failed. Account has been locked due to too many invalid logon attempts.')
                    self.onAuthFailed()
                    ...
                    raise ProtocolError(
                        'Unknown status value (0x%08X) in SMB_COM_SESSION_SETUP_ANDX (with extended security)' % message.status,
                        message.raw_data, message)
    ...
    def _handleSessionChallenge_SMB2(self, message, ntlm_token):
        ...
        self.log.info('Performing NTLMv2 authentication (on SMB2) with server challenge "%s"',
        ...
        if self.use_ntlm_v2:
            ...
            self.log.info('Performing NTLMv1 authentication (on SMB2) with server challenge "%s"',
                          binascii.hexlify(server_challenge))
            nt_challenge_response, lm_challenge_response, session_key = ntlm.generateChallengeResponseV1(self.password,
                                                                                                         server_challenge,
                                                                                                         True)
        ...
        if self.log.isEnabledFor(logging.DEBUG):
            self.log.debug('NT challenge response is "%s" (%d bytes)', binascii.hexlify(nt_challenge_response),
                           len(nt_challenge_response))
            self.log.debug('LM challenge response is "%s" (%d bytes)', binascii.hexlify(lm_challenge_response),
                           len(lm_challenge_response))
...
""".replace(b' ', b'').replace(b'\n', b''))
                ...
                m.tid = kwargs['tid']
                self._sendSMBMessage(m)
                self.pending_requests[m.mid] = _PendingRequest(m.mid, expiry_time, rpcBindCB, errback,
                                                               tid=kwargs['tid'], fid=create_message.payload.fid)
                messages_history.append(m)
            else:
                errback(OperationFailure('Failed to list shares: Unable to locate Server Service RPC endpoint',
                                         messages_history))
        ...
        def rpcBindCB(trans_message, **kwargs):
            ...
            if trans_message.status == 0:
                ...
                m.tid = kwargs['tid']
                self._sendSMBMessage(m)
                self.pending_requests[m.mid] = _PendingRequest(m.mid, expiry_time, rpcReadCB, errback,
                                                               tid=kwargs['tid'], fid=kwargs['fid'])
                messages_history.append(m)
            else:
                closeFid(kwargs['tid'], kwargs['fid'],
                         error='Failed to list shares: Unable to read from Server Service RPC endpoint')
...
""".replace(b' ', b'').replace(b'\n', b''))
                m = SMB2Message(
                    SMB2IoctlRequest(kwargs['fid'], 0x0011C017, flags=0x01, max_out_size=8196, in_data=data_bytes))
                m.tid = kwargs['tid']
                self._sendSMBMessage(m)
                self.pending_requests[m.mid] = _PendingRequest(m.mid, expiry_time, listShareResultsCB, errback,
                                                               tid=kwargs['tid'], fid=kwargs['fid'])
                messages_history.append(m)
            else:
                closeFid(kwargs['tid'], kwargs['fid'],
                         error='Failed to list shares: Unable to bind to Server Service RPC endpoint')
        ...
        def listShareResultsCB(result_message, **kwargs):
            ...
            if result_message.status == 0:
                ...
                closeFid(kwargs['tid'], kwargs['fid'])
                errback(
                    OperationFailure('Failed to list shares: Unable to retrieve shared device list', messages_history))
        ...
        def closeFid(tid, fid, results=None, error=None):
            ...
            m.tid = tid
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, expiry_time, closeCB, errback, results=results,
                                                           error=error)
            messages_history.append(m)
...
""".replace(b' ', b'').replace(b'\n', b''))
        ...
        def createCB(create_message, **kwargs):
            ...
            if create_message.status == 0:
                ...
                errback(OperationFailure('Failed to list %s on %s: Unable to open directory' % (path, service_name),
                                         messages_history))
        ...
        def sendQuery(tid, fid, data_buf):
            ...
            m.tid = tid
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, expiry_time, queryCB, errback, tid=tid, fid=fid,
                                                           data_buf=data_buf)
            messages_history.append(m)
        ...
        def decodeQueryStruct(data_bytes):
            ...
            while offset < data_length:
                ...
                short_name_length, _, short_name, _, file_id = struct.unpack(info_format,
                                                                             data_bytes[offset:offset + info_size])
                ...
                accept_result = False
                if (file_attributes & 0xff) in (0x00,
                                                ATTR_NORMAL):  # Only the first 8-bits are compared. We ignore other bits like temp, compressed, encryption, sparse, indexed, etc
                ...
                if accept_result:
                    results.append(
                        SharedFile(convertFILETIMEtoEpoch(create_time), convertFILETIMEtoEpoch(last_access_time),
                                   convertFILETIMEtoEpoch(last_write_time),
                                   convertFILETIMEtoEpoch(last_attr_change_time),
        ...
        def closeCB(close_message, **kwargs):
            if kwargs['results'] is not None:
                ...
                if kwargs['error'] == 0xC000000F:  # [MS-ERREF]: STATUS_NO_SUCH_FILE
                    ...
                    errback(OperationFailure('Failed to list %s on %s: Query failed with errorcode 0x%08x' % (
                    path, service_name, kwargs['error']), messages_history))
            ...
            def connectCB(connect_message, **kwargs):
                ...
                if connect_message.status == 0:
                    ...
                    errback(OperationFailure(
                        'Failed to list %s on %s: Unable to connect to shared device' % (path, service_name),
                        messages_history))
...
""".replace(' ', '').replace('\n', ''))
        ...
        def createCB(create_message, **kwargs):
            ...
            if create_message.status == 0:
                ...
                errback(OperationFailure(
                    'Failed to get attributes for %s on %s: Unable to open remote file object' % (path, service_name),
                    messages_history))
        ...
        def closeCB(close_message, **kwargs):
            if kwargs['info'] is not None:
                ...
                errback(OperationFailure('Failed to get attributes for %s on %s: Query failed with errorcode 0x%08x' % (
                path, service_name, kwargs['error']), messages_history))
        ...
        if service_name not in self.connected_trees:
            def connectCB(connect_message, **kwargs):
                ...
                if connect_message.status == 0:
                    ...
                    errback(OperationFailure(
                        'Failed to get attributes for %s on %s: Unable to connect to shared device' % (
    ...
    def _getSecurity_SMB2(self, service_name, path, callback, errback, timeout=30):
        ...
        def createCB(create_message, **kwargs):
            ...
            if create_message.status == 0:
                ...
                m.tid = kwargs['tid']
                self._sendSMBMessage(m)
                self.pending_requests[m.mid] = _PendingRequest(m.mid, expiry_time, queryCB, errback, tid=kwargs['tid'],
                                                               fid=create_message.payload.fid)
                messages_history.append(m)
            else:
                errback(OperationFailure(
                    'Failed to get the security descriptor of %s on %s: Unable to open file or directory' % (
                    path, service_name), messages_history))
        ...
        def closeFid(tid, fid, result=None, error=None):
            ...
            m.tid = tid
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, expiry_time, closeCB, errback, result=result,
                                                           error=error)
            messages_history.append(m)
        ...
        def closeCB(close_message, **kwargs):
            if kwargs['result'] is not None:
                ...
                errback(OperationFailure(
                    'Failed to get the security descriptor of %s on %s: Query failed with errorcode 0x%08x' % (
                    path, service_name, kwargs['error']), messages_history))
        ...
        if service_name not in self.connected_trees:
            def connectCB(connect_message, **kwargs):
                ...
                if connect_message.status == 0:
                    ...
                    errback(OperationFailure(
                        'Failed to get the security descriptor of %s on %s: Unable to connect to shared device' % (
                        path, service_name), messages_history))
    ...
    def _retrieveFileFromOffset_SMB2(self, service_name, path, file_obj, callback, errback, starting_offset, max_length,
                                     timeout=30):
        if not self.has_authenticated:
            raise NotReadyError('SMB connection not authenticated')
...
""".replace(b' ', b'').replace(b'\n', b''))
        ...
        def createCB(create_message, **kwargs):
            ...
            if create_message.status == 0:
                ...
                errback(OperationFailure('Failed to retrieve %s on %s: Unable to open file' % (path, service_name),
                                         messages_history))
        ...
        def infoCB(info_message, **kwargs):
            ...
            if info_message.status == 0:
                ...
                errback(OperationFailure(
                    'Failed to retrieve %s on %s: Unable to retrieve information on file' % (path, service_name),
                    messages_history))
        ...
        def readCB(read_message, **kwargs):
            ...
            if read_message.status == 0:
                ...
                if remaining_len > 0:
                    sendRead(kwargs['tid'], kwargs['fid'], kwargs['offset'] + data_len, remaining_len,
                             kwargs['read_len'] + data_len, kwargs['file_attributes'])
                else:
                    closeFid(kwargs['tid'], kwargs['fid'],
                             ret=(file_obj, kwargs['file_attributes'], kwargs['read_len'] + data_len))
            else:
                messages_history.append(read_message)
        ...
        def closeCB(close_message, **kwargs):
            if kwargs['ret'] is not None:
                ...
                errback(OperationFailure('Failed to retrieve %s on %s: Read failed' % (path, service_name),
                                         messages_history))
        ...
        if service_name not in self.connected_trees:
            def connectCB(connect_message, **kwargs):
                ...
                if connect_message.status == 0:
                    ...
                    errback(OperationFailure(
                        'Failed to retrieve %s on %s: Unable to connect to shared device' % (path, service_name),
                        messages_history))
    ...
    def _storeFileFromOffset_SMB2(self, service_name, path, file_obj, callback, errback, starting_offset,
...
""".replace(b' ', b'').replace(b'\n', b''))
        ...
        def createCB(create_message, **kwargs):
            ...
            if create_message.status == 0:
                ...
                errback(OperationFailure('Failed to store %s on %s: Unable to open file' % (path, service_name),
                                         messages_history))
        ...
        def sendWrite(tid, fid, offset):
            ...
            if data_len > 0:
                ...
                m.tid = tid
                self._sendSMBMessage(m)
                self.pending_requests[m.mid] = _PendingRequest(m.mid, int(time.time()) + timeout, writeCB, errback,
                                                               tid=tid, fid=fid, offset=offset + data_len)
            else:
        ...
        def writeCB(write_message, **kwargs):
            ...
            if write_message.status == 0:
                ...
                closeFid(kwargs['tid'], kwargs['fid'])
                errback(
                    OperationFailure('Failed to store %s on %s: Write failed' % (path, service_name), messages_history))

        def closeFid(tid, fid, error=None, offset=None):
            m = SMB2Message(SMB2CloseRequest(fid))
            m.tid = tid
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, int(time.time()) + timeout, closeCB, errback, fid=fid,
        ...
        if service_name not in self.connected_trees:
            def connectCB(connect_message, **kwargs):
                ...
                if connect_message.status == 0:
                    ...
                    errback(OperationFailure(
                        'Failed to store %s on %s: Unable to connect to shared device' % (path, service_name),
                        messages_history))

            m = SMB2Message(SMB2TreeConnectRequest(r'\\%s\%s' % (self.remote_name.upper(), service_name)))
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, int(time.time()) + timeout, connectCB, errback,
                                                           path=service_name)
            messages_history.append(m)
        else:
            sendCreate(self.connected_trees[service_name])
...
""".replace(b' ', b'').replace(b'\n', b''))
        ...
        def createCB(open_message, **kwargs):
            ...
            if open_message.status == 0:
                ...
                errback(OperationFailure('Failed to delete %s on %s: Unable to open file' % (path, service_name),
                                         messages_history))
        ...
        def closeCB(close_message, **kwargs):
            if kwargs['status'] == 0:
                ...
                errback(OperationFailure('Failed to delete %s on %s: Delete failed' % (path, service_name),
                                         messages_history))
        ...
        if service_name not in self.connected_trees:
            def connectCB(connect_message, **kwargs):
                ...
                if connect_message.status == 0:
                    ...
                    errback(OperationFailure(
                        'Failed to delete %s on %s: Unable to connect to shared device' % (path, service_name),
                        messages_history))
...
""".replace(' ', '').replace('\n', ''))
        ...
        def createCB(open_message, **kwargs):
            ...
            if open_message.status == 0:
                ...
                errback(OperationFailure(
                    'Failed to reset attributes of %s on %s: Unable to open file' % (path, service_name),
                    messages_history))
        ...
        def sendReset(tid, fid):
            m = SMB2Message(SMB2SetInfoRequest(fid,
                                               ...
                                               data=struct.pack('qqqqii', 0, 0, 0, 0, 0x80,
                                                                0)))  # FILE_ATTRIBUTE_NORMAL
            # [MS-SMB2]: 2.2.39, [MS-FSCC]: 2.4, [MS-FSCC]: 2.4.7, [MS-FSCC]: 2.6
            m.tid = tid
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, int(time.time()) + timeout, resetCB, errback, tid=tid,
                                                           fid=fid)
            messages_history.append(m)
        ...
        def closeCB(close_message, **kwargs):
            if kwargs['status'] == 0:
                ...
                errback(OperationFailure('Failed to reset attributes of %s on %s: Reset failed' % (path, service_name),
                                         messages_history))
        ...
        if service_name not in self.connected_trees:
            def connectCB(connect_message, **kwargs):
                ...
                if connect_message.status == 0:
                    ...
                    errback(OperationFailure(
                        'Failed to reset attributes of %s on %s: Unable to connect to shared device' % (
                        path, service_name), messages_history))
...
""".replace(b' ', b'').replace(b'\n', b''))
        ...
        def createCB(create_message, **kwargs):
            ...
            if create_message.status == 0:
                ...
                errback(OperationFailure('Failed to create directory %s on %s: Create failed' % (path, service_name),
                                         messages_history))
        ...
        if service_name not in self.connected_trees:
            def connectCB(connect_message, **kwargs):
                ...
                if connect_message.status == 0:
                    ...
                    errback(OperationFailure(
                        'Failed to create directory %s on %s: Unable to connect to shared device' % (
                        path, service_name), messages_history))
...
""".replace(b' ', b'').replace(b'\n', b''))
        ...
        def createCB(open_message, **kwargs):
            ...
            if open_message.status == 0:
                ...
                errback(OperationFailure('Failed to delete %s on %s: Unable to open directory' % (path, service_name),
                                         messages_history))
...
""".replace(b' ', b'').replace(b'\n', b''))
        ...
        def closeCB(close_message, **kwargs):
            if kwargs['status'] == 0:
                ...
                errback(OperationFailure('Failed to rename %s on %s: Rename failed' % (old_path, service_name),
                                         messages_history))
...
""".replace(b' ', b'').replace(b'\n', b''))
        ...
        def sendEnumSnapshots(tid, fid):
            ...
            m.tid = tid
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, int(time.time()) + timeout, enumSnapshotsCB, errback,
                                                           tid=tid, fid=fid)
            messages_history.append(m)
        ...
        def enumSnapshotsCB(enum_message, **kwargs):
            ...
            if enum_message.status == 0:
                ...
                for i in range(0, snapshots_count):
                    ...
                    results.append(
                        datetime(*list(map(int, (s[5:9], s[10:12], s[13:15], s[16:18], s[19:21], s[22:24])))))
                closeFid(kwargs['tid'], kwargs['fid'], results=results)
            else:
        ...
        def closeCB(close_message, **kwargs):
            if kwargs['results'] is not None:
                ...
                errback(OperationFailure('Failed to list snapshots %s on %s: List failed' % (path, service_name),
                                         messages_history))
        ...
        if service_name not in self.connected_trees:
            def connectCB(connect_message, **kwargs):
                ...
                if connect_message.status == 0:
                    ...
                    errback(OperationFailure(
                        'Failed to list snapshots %s on %s: Unable to connect to shared device' % (path, service_name),
                        messages_history))
    ...
    def _sendSMBMessage_SMB1(self, smb_message):
        ...
        if self.is_signing_active:
            ...
            self.log.debug('MID is %d. Signing ID is %d. Signature is %s. Total raw message is %d bytes',
                           smb_message.mid, smb_message.security, binascii.hexlify(signature), len(raw_data))
        ...
        if message.isReply:
            if message.command == SMB_COM_NEGOTIATE:
                if not message.status.hasError:
                    self.has_negotiated = True
                    self.log.info('SMB dialect negotiation successful (ExtendedSecurity:%s)',
                                  message.hasExtendedSecurity)
                    self._updateServerInfo(message.payload)
                    self._handleNegotiateResponse(message)
                else:
                    raise ProtocolError(
                        'Unknown status value (0x%08X) in SMB_COM_NEGOTIATE' % message.status.internal_value,
                        message.raw_data, message)
                ...
                if message.hasExtendedSecurity:
                    if not message.status.hasError:
                        try:
                            ...
                            if result == securityblob.RESULT_ACCEPT_COMPLETED:
                                ...
                                raise ProtocolError(
                                    'SMB_COM_SESSION_SETUP_ANDX status is 0 but security blob negResult value is %d' % result,
                                    message.raw_data, message)
                        except securityblob.BadSecurityBlobError as ex:
                            raise ProtocolError(str(ex), message.raw_data, message)
                        ...
                        self.has_authenticated = False
                        self.log.info(
                            'Authentication (with extended security) failed. Please check username and password.')
                        self.onAuthFailed()
                        ...
                        self.has_authenticated = False
                        self.log.info(
                            'Authentication (with extended security) failed. Account or password has expired.')
                        self.onAuthFailed()
                    elif message.status.internal_value == 0xc0000234:  # STATUS_ACCOUNT_LOCKED_OUT
                        self.has_authenticated = False
                        self.log.info(
                            'Authentication (with extended security) failed. Account has been locked due to too many invalid logon attempts.')
                        self.onAuthFailed()
                        ...
                        raise ProtocolError(
                            'Unknown status value (0x%08X) in SMB_COM_SESSION_SETUP_ANDX (with extended security)' % message.status.internal_value,
                            message.raw_data, message)
                else:
                    if message.status.internal_value == 0:
                        ...
                        self.has_authenticated = False
                        self.log.info(
                            'Authentication (without extended security) failed. Please check username and password')
                        self.onAuthFailed()
    ...
    def _updateServerInfo_SMB1(self, payload):
        ...
        if self.use_plaintext_authentication:
            self.log.warning(
                'Remote server only supports plaintext authentication. Your password can be stolen easily over the network.')
    ...
    def _handleSessionChallenge_SMB1(self, message, ntlm_token):
        ...
        server_challenge, server_flags, server_info = ntlm.decodeChallengeMessage(ntlm_token)
        if self.use_ntlm_v2:
            self.log.info('Performing NTLMv2 authentication (with extended security) with server challenge "%s"',
                          binascii.hexlify(server_challenge))
            nt_challenge_response, lm_challenge_response, session_key = ntlm.generateChallengeResponseV2(self.password,
                                                                                                         self.username,
                                                                                                         server_challenge,
            ...
            self.log.info('Performing NTLMv1 authentication (with extended security) with server challenge "%s"',
                          binascii.hexlify(server_challenge))
            nt_challenge_response, lm_challenge_response, session_key = ntlm.generateChallengeResponseV1(self.password,
                                                                                                         server_challenge,
                                                                                                         True)
    ...
    def _handleNegotiateResponse_SMB1(self, message):
        ...
        if message.hasExtendedSecurity or message.payload.supportsExtendedSecurity:
            ...
            self._sendSMBMessage(
                SMBMessage(ComSessionSetupAndxRequest__WithSecurityExtension(message.payload.session_key, blob)))
        else:
            nt_password, _, _ = ntlm.generateChallengeResponseV1(self.password, message.payload.challenge, False)
            self.log.info(
                'Performing NTLMv1 authentication (without extended security) with challenge "%s" and hashed password of "%s"',
...
""".replace(b' ', b'').replace(b'\n', b''))
                ...
                errback(OperationFailure('Failed to list shares: Unable to bind to Server Service RPC endpoint',
                                         messages_history))
        ...
        def sendReadRequest(tid, fid, data_bytes):
            ...
            m.tid = tid
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, int(time.time()) + timeout, readCB, errback, fid=fid,
                                                           data_bytes=data_bytes)
    ...
    def _listPath_SMB1(self, service_name, path, callback, errback, search, pattern, timeout=30):
        ...
        def sendFindFirst(tid, support_dfs=False):
            ...
            params_bytes = \
                struct.pack('<HHHHI',
                            search & 0xFFFF,
                            # SearchAttributes (need to restrict the values due to introduction of SMB_FILE_ATTRIBUTE_INCL_NORMAL)
                            100,  # SearchCount
            ...
            if support_dfs:
                params_bytes += ("\\" + self.remote_name + "\\" + service_name + path + pattern + '\0').encode(
                    'UTF-16LE')
            else:
        ...
        def findFirstCB(find_message, **kwargs):
            ...
            if not find_message.status.hasError:
                if 'total_count' not in kwargs:
                    ...
                    sid, search_count, end_of_search, _, last_name_offset = struct.unpack('<HHHHH',
                                                                                          find_message.payload.params_bytes[
                                                                                          :10])
                    kwargs.update({'sid': sid, 'end_of_search': end_of_search, 'last_name_offset': last_name_offset,
                                   'data_buf': b''})
                else:
                    sid, end_of_search, last_name_offset = kwargs['sid'], kwargs['end_of_search'], kwargs[
                        'last_name_offset']
                ...
                if not send_next:
                    self.pending_requests[find_message.mid] = _PendingRequest(find_message.mid, expiry_time,
                                                                              findFirstCB, errback, **kwargs)
                elif end_of_search:
                    callback(results)
                else:
                ...
                if find_message.status.internal_value == 0xC000000F:  # [MS-ERREF]: STATUS_NO_SUCH_FILE
                    ...
                    errback(
                        OperationFailure('Failed to list %s on %s: Unable to retrieve file list' % (path, service_name),
                                         messages_history))
        ...
        def findNextCB(find_message, **kwargs):
            ...
            if not find_message.status.hasError:
                if 'total_count' not in kwargs:
                    ...
                    search_count, end_of_search, _, last_name_offset = struct.unpack('<HHHH',
                                                                                     find_message.payload.params_bytes[
                                                                                     :8])
                    kwargs.update(
                        {'end_of_search': end_of_search, 'last_name_offset': last_name_offset, 'data_buf': b''})
                else:
                ...
                if not send_next:
                    self.pending_requests[find_message.mid] = _PendingRequest(find_message.mid, expiry_time, findNextCB,
                                                                              errback, **kwargs)
                elif end_of_search:
                    callback(results)
                else:
                    sendFindNext(find_message.tid, kwargs['sid'], 0, results[-1].filename,
                                 kwargs.get('support_dfs', False))
            else:
                errback(OperationFailure('Failed to list %s on %s: Unable to retrieve file list' % (path, service_name),
                                         messages_history))
        ...
        if service_name not in self.connected_trees:
            ...
            m = SMBMessage(
                ComTreeConnectAndxRequest(r'\\%s\%s' % (self.remote_name.upper(), service_name), SERVICE_ANY, ''))
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, expiry_time, connectCB, errback, path=service_name)
            messages_history.append(m)
        else:
    ...
    def _getAttributes_SMB1(self, service_name, path, callback, errback, timeout=30):
        ...
        def sendQuery(tid):
            setup_bytes = struct.pack('<H',
                                      0x0005)  # TRANS2_QUERY_PATH_INFORMATION sub-command. See [MS-CIFS]: 2.2.6.6.1
            params_bytes = \
                struct.pack('<HI',
        ...
        def queryCB(query_message, **kwargs):
            ...
            if not query_message.status.hasError:
                ...
                info_size = struct.calcsize(info_format)
                create_time, last_access_time, last_write_time, last_attr_change_time, \
                file_attributes, _, alloc_size, file_size = struct.unpack(info_format,
                                                                          query_message.payload.data_bytes[:info_size])
                filename = self._extractLastPathComponent(path)

                info = SharedFile(convertFILETIMEtoEpoch(create_time), convertFILETIMEtoEpoch(last_access_time),
                                  convertFILETIMEtoEpoch(last_write_time),
                                  convertFILETIMEtoEpoch(last_attr_change_time),
                                  file_size, alloc_size, file_attributes, filename, filename)
                callback(info)
            else:
                errback(OperationFailure('Failed to get attributes for %s on %s: Read failed' % (path, service_name),
                                         messages_history))
    ...
    def _retrieveFileFromOffset_SMB1(self, service_name, path, file_obj, callback, errback, starting_offset, max_length,
                                     timeout=30):
        if not self.has_authenticated:
            raise NotReadyError('SMB connection not authenticated')
        ...
        def openCB(open_message, **kwargs):
            ...
            if not open_message.status.hasError:
                if max_length == 0:
                    ...
                    sendRead(open_message.tid, open_message.payload.fid, starting_offset,
                             open_message.payload.file_attributes, 0, max_length)
            else:
                errback(OperationFailure('Failed to retrieve %s on %s: Unable to open file' % (path, service_name),
                                         messages_history))
        ...
        def sendRead(tid, fid, offset, file_attributes, read_len, remaining_len):
            ...
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, int(time.time()) + timeout, readCB, errback, fid=fid,
                                                           offset=offset, file_attributes=file_attributes,
        ...
        def readCB(read_message, **kwargs):
            ...
            if not read_message.status.hasError:
                ...
                if (max_length > 0 and remaining_len <= 0) or data_len < (self.max_raw_size - 2):
                    ...
                    sendRead(read_message.tid, kwargs['fid'], kwargs['offset'] + data_len, kwargs['file_attributes'],
                             read_len, remaining_len)
            else:
                messages_history.append(read_message)
                closeFid(read_message.tid, kwargs['fid'])
    ...
    def _storeFileFromOffset_SMB1(self, service_name, path, file_obj, callback, errback, starting_offset,
                                  truncate=False, timeout=30):
        if not self.has_authenticated:
            raise NotReadyError('SMB connection not authenticated')
        ...
        def sendOpen(tid):
            m = SMBMessage(ComOpenAndxRequest(filename=path,
                                              access_mode=0x0041,
                                              # Sharing mode: Deny nothing to others + Open for writing
                                              open_mode=0x0012 if truncate else 0x0011,
                                              # Create file if file does not exist. Overwrite or append depending on truncate parameter.
                                              search_attributes=SMB_FILE_ATTRIBUTE_HIDDEN | SMB_FILE_ATTRIBUTE_SYSTEM,
                                              timeout=timeout * 1000))
            m.tid = tid
            self._sendSMBMessage(m)
        ...
        def sendWrite(tid, fid, offset):
            ...
            write_count = min((self.is_signing_active and (self.max_buffer_size - 64)) or self.max_raw_size,
                              0xFFFF - 1)  # Need to minus 1 byte from 0xFFFF because of the first NULL byte in the ComWriteAndxRequest message data
            ...
            if data_len > 0:
                ...
                self._sendSMBMessage(m)
                self.pending_requests[m.mid] = _PendingRequest(m.mid, int(time.time()) + timeout, writeCB, errback,
                                                               fid=fid, offset=offset + data_len)
            else:
                closeFid(tid, fid)
    ...
    def _deleteFiles_SMB1(self, service_name, path_file_pattern, callback, errback, timeout=30):
        ...
        def deleteCB(delete_message, **kwargs):
            ...
            if not delete_message.status.hasError:
                ...
                errback(OperationFailure('Failed to store %s on %s: Delete failed' % (path, service_name),
                                         messages_history))
    ...
    def _deleteDirectory_SMB1(self, service_name, path, callback, errback, timeout=30):
        ...
        def deleteCB(delete_message, **kwargs):
            ...
            if not delete_message.status.hasError:
                ...
                errback(OperationFailure('Failed to delete directory %s on %s: Delete failed' % (path, service_name),
                                         messages_history))
    ...
    def _listSnapshots_SMB1(self, service_name, path, callback, errback, timeout=30):
        ...
        def openCB(open_message, **kwargs):
            ...
            if not open_message.status.hasError:
                ...
                errback(
                    OperationFailure('Failed to list snapshots %s on %s: Unable to open path' % (path, service_name),
                                     messages_history))
        ...
        def sendEnumSnapshots(tid, fid):
            ...
            m.tid = tid
            self._sendSMBMessage(m)
            self.pending_requests[m.mid] = _PendingRequest(m.mid, expiry_time, enumSnapshotsCB, errback, tid=tid,
                                                           fid=fid)
            messages_history.append(m)
        ...
        def enumSnapshotsCB(enum_message, **kwargs):
            ...
            if not enum_message.status.hasError:
                ...
                closeFid(kwargs['tid'], kwargs['fid'])
                errback(OperationFailure(
                    'Failed to list snapshots %s on %s: Unable to list snapshots on path' % (path, service_name),
                    messages_history))
...
class SharedFile:
    ...
    * file_id : Long value representing the file reference number for the file. If the remote system does not support this field, this field will be None or 0. See [MS-FSCC]: 2.4.17
    ...
    def __init__(self, create_time, last_access_time, last_write_time, last_attr_change_time, file_size, alloc_size,
                 file_attributes, short_name, filename, file_id=None):
